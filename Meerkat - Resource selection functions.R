...
# RESOURCE SELECTIONS FUNCTIONS (HABITAT SELECTION AT LANDSCAPE SCALE) 
...
modDF <-select(tracks_df, SessionID, GroupRef, Date, Time, habitat, GPS_Longitude, GPS_Latitude)
...
# Plot representaiton across the morning
...
# Sub Sample Burrow and Foraging Period points
  ...
  BurrowDF <- filter(modDF, PointOrder==1)    
...
# For the foraging track we sample between 1 & 4 hours from the first burrow point
  ...
  ForageDF <- filter(modDF, ForTime >= 60 & ForTime <= 240) %>% 
    group_by(SessionID, GroupRef, Date) %>%
    slice_sample(n=1)  
  
  ggplot(BurrowDF,aes(x = habitat, fill = habitat)) +
    geom_bar(stat = "count") 
  ggplot(ForageDF, aes(x = habitat, fill = habitat)) +
        geom_bar(stat="count")
...
for(i in 1:nrow(GrpSeason_Summary)){
  ...
  BoundPoints <- st_as_sf(filter(PerSession_DF, GroupRef == GrpRef), coords = c("X","Y"), crs = 32734)
  
  GroupSeason_Boundaries_list[[i]] <- fm_nonconvex_hull(BoundPoints, convex = 0.5, concave = -0.5) %>% 
    st_as_sf() %>% 
...
pointMerge_Burrow_HAB <- st_as_sf(points_sf) %>% 
  mutate(habitat = ifelse(is.na(Class_Name) == T, "OutOfBounds", as.character(Class_Name)))
...
modDF_Burrow <- data.frame(pointMerge_Burrow_HAB) %>% 
  ...
  filter(habitat!="OutOfBounds")

Boundary2 <- fm_nonconvex_hull(filter(pointMerge_Burrow_HAB, habitat != "OutOfBounds"), 
                               convex = 0.1, concave = -0.1)
plot(Boundary2)

inla_mesh2 <- fm_mesh_2d(loc = filter(pointMerge_Burrow_HAB, habitat != "OutOfBounds"), # coordinates
...
modDF_Burrow <- modDF_Burrow %>% 
  mutate(wt = case_when(Present == 1 ~ 1e-6,
                        Present == 0 ~ as.numeric((st_area(Boundary2))/nrow(filter(modDF_Burrow, Present==0)))))
...
BurrowSDM_RF <- sdmTMB(Present/wt ~ habitat*season + (1|GroupRef),
                       data = modDF_Burrow,
...
plot(emmeans(BurrowSDM_RF, pairwise~habitat, by = "season"))

# Burrow SDM with spatial term 
BurrowSDM_spatio <- sdmTMB(Present/wt ~ habitat*season + (1|GroupRef),
                           data = modDF_Burrow,
                           family = poisson(link="log"),
                           mesh = mesh_Burrow,
                           weights = modDF_Burrow$wt,
...
plot(emmeans(BurrowSDM_spatio, pairwise~habitat, by="season"))

# Extract pairwise contrast from the model using emmeans
BurrowContrast <- emmeans(BurrowSDM_spatio, pairwise ~ habitat, by = "season")
...
plot1 <- ggplot(filter(Contrasts_Burrow, Group1 == "Drie Doring"), aes(x = Group2, y = exp(estimate))) +
...
burrow_means <- data.frame(emmeans(BurrowSDM_spatio, ~ habitat + season)) %>% 
  mutate(selection_intensity = exp(emmean)) %>% 
  select(habitat, season, selection_intensity)
...
table(pointMerge_Forage_HAB$habitat[pointMerge_Forage_HAB$Present == 1])
table(pointMerge_Forage_HAB$habitat[pointMerge_Forage_HAB$Present == 0])
...
# create mesh for each
  ...
  Boundary <- fm_nonconvex_hull(filter(pointMerge_Forage_HAB, habitat != "OutOfBounds"), 
                                 convex = 0.1, concave = -0.1)
  plot(Boundary)
  
# Generate mesh using inla based on the boundary  
inla_mesh <- fm_mesh_2d(loc = filter(pointMerge_Forage_HAB, habitat != "OutOfBounds"), # coordinates
  max.edge = c(0.6, 1), # max triangle edge length; inner and outer meshes
  offset = c(0.7,2),  # inner and outer border widths
  cutoff = 0.4, # minimum triangle edge length
  boundary = Boundary)
...
# generate zero weight values
modDF_Forage <- modDF_Forage %>% mutate(wt = case_when(Present == 1 ~ 1e-6,
                                                      Present == 0 ~ as.numeric((st_area(Boundary))/nrow(filter(modDF_Forage, Present==0)))))
...
# Base model
ForageSDM_base <- sdmTMB(Present/wt ~ habitat*season,
                    data = modDF_Forage,
                    family = poisson(link="log"),
                    mesh = mesh_FOR,
...
plot(emmeans(ForageSDM_base, pairwise ~ habitat, by = "season"))
  
# Random Effects
ForageSDM_RF <- sdmTMB(Present/wt ~ habitat*season + (1|GroupRef),
                    data = modDF_Forage,
                    family = poisson(link="log"),
                    mesh = mesh_FOR,
                    weights = modDF_Forage$wt,
...
plot(emmeans(ForageSDM_RF, pairwise ~ habitat, by="season"))
  
# Random effects + Spatial Variation
ForageSDM_spatio <- sdmTMB(Present/wt ~ habitat*season + (1|GroupRef),
                        data = modDF_Forage,
                        family = poisson(link="log"),
                        mesh = mesh_FOR,
                        weights = modDF_Forage$wt,
...
plot(emmeans(ForageSDM_spatio,~ habitat + season))
ForContrast <- emmeans(ForageSDM_spatio, pairwise ~ habitat, by="season")
...
plot2 <- ggplot(Contrasts_Forage %>% filter(Group1 == "Drie Doring"),
...
forage_means <- data.frame(emmeans(ForageSDM_spatio, ~ habitat + season)) %>% 
  mutate(selection_intensity = exp(emmean)) %>% 
  select(habitat, season, selection_intensity)
...
#saveRDS(plot1, "plot_RSFburrow_season.RDS")
#saveRDS(plot2, "plot_RSFforaging_season.RDS")
